module renderer.sdl2.gles2;

import core.stdc.string;
import bindbc.opengl;
import bindbc.sdl;
import ddui;
import skeleton : window, window_height, window_width;
...
__gshared GLushort[BUFFER_SIZE * 6] index_buf;
__gshared int buf_idx;

void initiate_renderer()
{
    GLSupport glstatus = loadOpenGL;
    assert(glstatus >= GLSupport.gl21, "Failed to load OpenGL 2.1 (needed for GLES 2.0 functions)");
    ...
    // Buffer setup (no VAO in GLES 2.0)
    glGenBuffers(1, &vbo);
    glGenBuffers(1, &ebo);
    ...
    // Upload atlas texture using GL_LUMINANCE (GLES 2.0 lacks GL_RED)
    glGenTextures(1, &font_texture);
    glBindTexture(GL_TEXTURE_2D, font_texture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, ATLAS_WIDTH, ATLAS_HEIGHT, 0,
        GL_LUMINANCE, GL_UNSIGNED_BYTE, atlas_texture.ptr);
...
void flush()
{
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, buf_idx * 6 * cast(int)GLushort.sizeof, index_buf.ptr, GL_STREAM_DRAW);

    // Set up vertex attrib pointers at draw time (no VAO)
    ...
    glDrawElements(GL_TRIANGLES, buf_idx * 6, GL_UNSIGNED_SHORT, null);

    glDisableVertexAttribArray(cast(GLuint)attrib_position);
    glDisableVertexAttribArray(cast(GLuint)attrib_texcoord);
    glDisableVertexAttribArray(cast(GLuint)attrib_color);
...
void push_quad(mu_Rect dst, mu_Rect src, mu_Color color)
{
    ...
    index_buf[ii + 0] = cast(GLushort)(vi + 0);
    index_buf[ii + 1] = cast(GLushort)(vi + 1);
    index_buf[ii + 2] = cast(GLushort)(vi + 2);
    index_buf[ii + 3] = cast(GLushort)(vi + 2);
    index_buf[ii + 4] = cast(GLushort)(vi + 3);
    index_buf[ii + 5] = cast(GLushort)(vi + 1);
}
...
{
    mu_Rect dst = mu_Rect(pos.x, pos.y, 0, 0);
    for (const(char) *p = text; *p; p++)
    {
        if ((*p & 0xc0) == 0x80) continue;
        int chr = mu_min(cast(ubyte) *p, 127);
